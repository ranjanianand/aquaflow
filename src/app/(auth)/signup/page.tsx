...
import { Loader2, AlertCircle, CheckCircle2, Eye, EyeOff, Shield, Droplets } from 'lucide-react';
import Image from 'next/image';

/**
 * Creating an account.
 *
 * Signing up grants nothing. A new account gets the lowest role and no plants
 * until an administrator raises it, and the card header says so before asking
 * for a password — better than letting somebody register and then wonder why
...
export default function SignupPage() {
  ...
  // Checked here so the message is immediate, and again by the API — a browser
  // check is a courtesy, not a control.
  const tooShort = password.length > 0 && password.length < 10;
  const mismatch = confirm.length > 0 && confirm !== password;
  const ready = Boolean(name.trim() && email.includes('@')
                        && password.length >= 10 && confirm === password);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // — which may be a failure or "check your email to confirm".
      if (message) {
        if (/could not|incorrect|already|invalid/i.test(message)) setError(message);
        ...
        <div className="bg-white border border-slate-200 shadow-xl rounded-2xl overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            ...
            <p className="text-xs text-slate-500 mt-1">
              An administrator grants plant access once your account exists
            </p>
          </div>
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {notice && (
                <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm rounded-lg">
              ...
              <div className="space-y-1.5">
                ...
                {tooShort && (
                  <p className="text-xs text-red-600">At least 10 characters.</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                />
                {mismatch && (
                  <p className="text-xs text-red-600">These do not match.</p>
                )}
              </div>
